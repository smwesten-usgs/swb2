...
!------------------------------------------------------------------------------  
  ...
  subroutine push_sub( this, grd )

    class(RUNNING_STATS_T), intent(inout)   :: this
    type(GENERAL_GRID_T), intent(in)        :: grd
    ...
    call assert( allocated( grd%dpData ), "Unallocated double-precision grid" )

    where ( grd%dpData > this%nodata_value )

      this%grd_delta%dpData = grd%dpData - this%grd_M1%dpData
